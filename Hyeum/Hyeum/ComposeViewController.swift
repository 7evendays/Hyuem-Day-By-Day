...
class ComposeViewController: UIViewController
{
    ...
    //보기 화면에서 편집 버튼을 탭하면 메모가 이 속성으로 전달
    //이 속성에 메모가 저장되어 있다면 편집 모드로 동작
    //목록 화면에서 + 버튼을 탭하면 전달되는 메모가 없음 즉, 속성이 nil이고 새 메모 쓰기 모드로 동작
...
//        Memo.dummyMemoList.append(newMemo)
        ...
        if let target = editTarget
        {
            ...
            //textVeiw에 입력되어 있느 값으로 메모를 바꾼 후 saveContext 매소드를 호출
            NotificationCenter.default.post(name: ComposeViewController.memoDidChange, object: nil)
        }
        else
        {
            DataManager.shared.addNewMemo(memo)
            //쓰기 모드라면 이전에 구현했던 코드 그대로 실행
            NotificationCenter.default.post(name: ComposeViewController.newMemoDidInsert, object: nil)
        }
        //편집 모드인 경우에는 새로운 메모를 추가하는 것이 아니라 편집된 내용을 저장
        //editTarget 속성을 기준으로 편집 모드를 처리
